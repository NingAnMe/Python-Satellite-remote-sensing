...
class convertModisGDAL:
    ...
    def _reprojectOne(self, l, quiet=False):
        ...
        try:
            dst_ds = self.driver.Create(out_name, self.dst_xsize,
                                        self.dst_ysize, 1, gdal.GDT_Float32)
        except:
    ...
    def run(self, quiet=False):
        ...
        if self.vrt:
            ...
            self._createWarped(self.layers)
            n = 0
            for i in self.subset:
                if str(i) == '1':
                    self._reprojectOne(self.layers, quiet=quiet)
                n = n + 1
            if not quiet:
...
def raster_copy_with_nodata(s_fh, s_xoff, s_yoff, s_xsize, s_ysize, s_band_n,
    ...
    data_tmp = None
    for i in range(s_fh.RasterCount):
        s_band = s_fh.GetRasterBand(i + 1)

        data_src = s_band.ReadAsArray(s_xoff, s_yoff, s_xsize, s_ysize,
                                      t_xsize, t_ysize)
        if data_tmp is None:
            data_tmp = np.zeros((data_src.shape[0], data_src.shape[1], s_fh.RasterCount), dtype=np.float)
        data_tmp[:, :, i] = data_src
    data_tmp[data_tmp == nodata] = np.nan
    data_src = np.nanmean(data_tmp, axis=2)
    # TODO
    data_src = data_src * 0.001
    data_src[np.isnan(data_src)] = nodata
    # data_src = data_src.astype(np.int32)
